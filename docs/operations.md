...
F1已提供文章CRUD与保存/并发/离开保护并通过验收；F2后端已提供唯一成功音频的PG bytea持久化、恢复播放下载、过期提示、倒序请求防覆盖和删除在途不复活。每阶段由产品验收并commit后放行下一阶段，不由后端单独提交。
...
### F2后端运行与验证（2026-09-10）

新增迁移1789056000000-article-audio.cjs随正常migrate运行，现有全库pg_dump -Fc自动包含文章、唯一音频metadata/bytea和迁移记录，不新增存储服务或部署端口。单份音频上限8MiB；失败保留旧成功行，删除文章级联音频。音频GET必须携带当前audioId，过期ID返回404；GET不调用供应商、不消耗合成入口配额。

隔离PG17演练脚本为apps/api/test/fixtures/backup-audio.mjs，要求显式TEST_DATABASE_URL及TEST_PG_CONTAINER，只对测试容器执行。演练创建独立恢复库后逐字核对正文、完整metadata及MP3字节，成功后删除测试账户和恢复库，临时dump保留在测试容器/tmp。结果PASS：40585字节，SHA256 db01b5519f1c7e859423bfe55f2a2e85ff86d598622f1bc68b7a30aa69133b67，应用迁移记录3条。此演练验证全库备份恢复路径，不代表已经远程执行生产备份脚本。

联调启动脚本apps/api/test/fixtures/serve-audio.mjs只读取显式TEST_DATABASE_URL，不加载生产.env；FIXTURE_PORT/FIXTURE_ORIGIN可配置，默认3002/4182。供应商transport阻止外部网络，仅返回apps/web/qa/tone.mp3完整可播放音调，不代表真实日文语音验收。FIXTURE_CONTROL_FILE可指定本地JSON文件（缺失或{}为正常；delayMs最多20000、fail、oversize模拟异常）。此脚本不进入生产启动链。

验证：完整后端6项通过、0跳过；路由大小写/尾斜杠同池限流及大写/V1错误或缺失Origin403的定向音频测试通过。应用重启后相同audioId、metadata和音频字节可恢复。Compose解析确认唯一127.0.0.1:8080→3000映射、db无宿主端口；现有运行栈映射一致。原8080未重建，GitHub Actions/VPS真实部署仍未执行。F2前端及产品验收均通过，按独立功能提交。
