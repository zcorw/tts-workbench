...
export function validateRule(word,reading) {
  if(!word.trim()||!reading.trim()) throw Error('词汇和读法都不能为空。');
  if([...word].length>100||[...reading].length>100) throw Error('词汇和读法各限 100 个字符。');
  if(/[<>\u0000-\u001f\u007f]/u.test(word+reading)) throw Error('请使用单行纯文本，不含控制字符或尖括号。');
}
