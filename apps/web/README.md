...
构建产物 `dist/` 使用根路径资源；生产预览为 `http://127.0.0.1:4182`。生产预览仅检查静态资源和路由，不将后端允许的开发 Origin 改为预览端口。正式部署由统一 Nginx 入口提供 `dist`、API 和音频，前端不直连供应商、不硬编码公网域名；同源 Cookie/CSRF 经统一入口使用。HashRouter 的 `#/workbench` 等地址可直接刷新，URL hash 不发送服务端。Compose/根配置由后端维护，本目录没有发布第二个生产宿主端口。
...
| 音频 | `AudioPlayer` | 真MP3 Blob、播放/暂停/停止、真实时长和进度、下载、旧音频过期、媒体失败 |
| 账户资料 | `AccountPage` | 当前账号、显示名称修改、个人规则数量、退出 |
| 新文章及使用指引 | `ArticleDialog` / `HelpDialog` | 业务说明与替换范围、示例/空白文章；没有开发参数或需求说明 |
...
| 路由 | React Router 7.18.3 | HashRouter兼容已验收导航与静态统一入口，无自建hashchange路由框架 |
...
官方依据：[Vite运行要求](https://vite.dev/guide/)、[React DOM内容编辑边界](https://react.dev/reference/react-dom/components/common)、[Radix Dialog](https://www.radix-ui.com/primitives/docs/components/dialog)、[React Hook Form](https://react-hook-form.com/get-started)、[RHF Zod resolver](https://github.com/react-hook-form/resolvers#zod)、[React Router HashRouter](https://reactrouter.com/api/declarative-routers/HashRouter)、[TanStack Query](https://tanstack.com/query/latest/docs/framework/react/reference/index)。已通过本机Node22.12安装、类型检查、构建及运行验证，精确版本以package-lock为准。

## 唯一接口契约

- 文件：`../../docs/openapi.yaml`，OpenAPI 3.0.3 / API **1.0.1**，17个操作。
- 当前 SHA256：`d27c2f7bd8560e2098215ee449922b48cc64057712416d2b58f0787a5d7e69e6`。
...
5. 正文合成只发原文input及契约参数，不发owner/tenant/dictionaryIds或预替换读法文本。草稿单词试听走`/v1/audio/preview`且不保存；列表已保存规则试听走正式合成。
...
7. 401清除账户缓存与音频，保留当前正文供同账户重新登录；正常退出和切换其他账户清理正文。其他账户不共享缓存。禁止自动重放计费请求。
...
详细记录与截图见 [QA报告](qa/REPORT.md)。真实联调脚本创建唯一随机测试账号，不读取已有私人账户、不输出或持久记录密码；用后会删除所创建的个人规则，测试账号留在隔离测试数据库。不要对未经授权的生产环境运行注册测试。
