...
- 文件：`../../docs/openapi.yaml`，OpenAPI 3.0.3 / API **1.1.0**，24个操作（F1接入前5个文章CRUD操作，2个持久音频操作留待F2）。
...
F1仍使用临时speech合成，生成前先保存；保存失败或期间再编辑则不生成。音频仅在当前文章页面提供，离开/刷新后需重新生成，界面已明确提示。**F2的持久音频生成、重开播放与唯一音频替换尚未实现**。单词候选试听继续不保存规则或文章音频。
