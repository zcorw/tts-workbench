...
- [x] P0：三方规划与唯一契约API1.1.0（24操作）、前端生成类型/指纹、产品语义核对；本项暂不表示运行实现可用。
- [x] F1：账户隔离文章CRUD完整垂直闭环：迁移、title/text验证、revision/contentRevision、MAX_ARTICLES默认100及原子容量、GET config、列表分页/详情/新建/保存/删除、409保留草稿、未保存离开保护。验收后产品commit。
- [x] F1验证：真实PG/HTTP测试跨账户404、标题trim/码点/正文字节边界、空草稿、并发保存/删除409、并发创建不超额和删除释放名额；前端切换与保存失败保留、重启后最新正文仍在。
- [x] F2：唯一成功音频完整垂直闭环：<=8MiB bytea原子替换、POST metadata/GET audioId二进制、恢复播放下载、实际规则快照及过期、last-success generationSeq、删除级联/在途不复活、同池计费限流/CSRF/取消。验收后产品commit。
- [x] F2验证：真实PG+可控供应商transport覆盖新请求先成功/新请求失败旧请求成功、正文变更与仅标题变更、规则在途改变、删除中途完成、失败保留、超8MiB、metadata到GET间替换404、跨账户不可下载及服务重启恢复。前端验证先保存且新编辑不误合成旧快照、404最多重取一次、原有规则和试听回归。
- [x] 综合验证与文档：备份恢复包含最新文章/唯一音频；Compose单loopback端口保持。付费真实试听只在明确授权下执行，不把夹具当可听验收；本增量不远程发布。

### F1后端交付记录（2026-09-10）

- [x] 后端文章迁移、5项CRUD接口、双版本/条件写入、账户隔离和原子容量、MAX_ARTICLES配置及GET config。F1暂返回audio=null、audioStale=false，F2接口未实现。
- [x] 后端全套5项测试通过、0跳过：包含真实HTTP/PG文章验证与原有认证、词汇和Gateway替身回归；覆盖迁移重入、并发创建/保存/删除、原文精确保留、跨账户404及应用重启恢复。
- [x] 产品独立HTTP验收7组通过：Unicode/trim、分页搜索、账户隔离、双版本、冲突/no-op、重新登录恢复、条件删除；没有调用付费供应商。
- [x] F1前端浏览器验收及产品统一review通过：文章22组、真实文章10组、原契约22组、原真实13组，以及password/build/check:api/format通过；产品独立HTTP7组和真实浏览器CRUD通过，源码及截图回读通过。F1已通过验收并提交2dd6d47，产品随后明确放行F2。

### F2后端交付记录（2026-09-10）

- [x] 唯一音频迁移/metadata和bytea原子替换、成功序号比较、实际规则快照和过期、条件生成、GET当前audioId、所有权、删除级联与在途不复活、锁后及提交前取消检查。
- [x] 完整后端6项通过（0跳过），并发乱序/后发失败/超8MiB保留/正文改回/仅标题/规则在途/锁等待取消/重启恢复；共享限流的大小写/尾斜杠及大写V1 Origin边界定向测试通过。
- [x] 可播放MP3隔离联调服务、文章与音频全库备份恢复精确一致、Compose单loopback入口检查通过；原8080保留，无付费调用或远程部署。
- [x] F2前端浏览器及产品独立验收、统一review通过，纳入独立功能提交。前端音频13/真实音频8/文章22/React22/真实文章10/真实账户规则13组通过，build/check:api/format通过。
